(()=>{
 ...
 const topic={ '001':'margin','002':'discount','003':'roas' }[id];
 if(topic){document.getElementById('tab-'+topic)?.click();const tabs=document.querySelector('.tabs');if(tabs)tabs.style.setProperty('display','none','important');}
 ...
 if(checklist){link('빈 워드 양식 (.docx)',`checklist-${id}.docx`);link('빈 인쇄 양식 (.pdf)',`checklist-${id}.pdf`)}else{link('이 글 전용 엑셀 (.xlsx)',`calculator-${id}.xlsx`)}
 const note=document.createElement('p');note.textContent=checklist?'다운로드 양식은 빈 양식입니다. 현재 체크와 메모를 보관하려면 아래 버튼을 누르세요.':`이 글의 계산만 담은 전용 엑셀입니다. 파일을 열고 노란 입력칸을 수정하세요. 웹 입력값은 자동으로 옮겨지지 않습니다.`;panel.append(note);
